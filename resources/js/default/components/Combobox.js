...
const setListItemAttributes = (currentListItem, input, listItems) => {
  ...
  // Update aria-activedescendant to assistive technologies
  input.setAttribute('aria-activedescendant', currentListItem.id || currentListItem.textContent.trim());
};
...
const initializeComboboxAutocomplete = (comboboxNode, listBoxNode, buttonNode) => {
  ...
  // Improved debounce function with better cleanup
  ...
  document.addEventListener('click', (event) => handleOutsideClick(event, comboboxNode, listBoxNode, buttonNode, input, state), { passive: true });
};
