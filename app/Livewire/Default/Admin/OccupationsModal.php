...
use App\Livewire\Forms\Occupation\AddForm;
use App\Livewire\Forms\Occupation\UpdateForm;
...
public function fieldGrades()
{
    ...
    public function updatedActiveOccupationId()
    {
        try {
            ...
            Occupation::where('user_id', $userId)->update(['is_active' => false]);
    ...
    public function mount()
    {
        ...
        $localeName = $this->locale === "en" ? "fr" : $this->locale;

        $this->loadSelectOptions();
        $this->employeeName = $this->user["name_{$localeName}"] ?? '';

        $activeOccupation = Occupation::where('user_id', $this->user['id'])
    ...
    {
        try {
            $occupation = Occupation::where('user_id', $this->user['id'])->findOrFail($occupationId);
...
{
    try {
        $user = $occupation->holder; // or ->user if you renamed the relation
        $occupation->delete();
    ...
    public function render()
    {
        ...
        return view('livewire.default.admin.occupations-modal');
    }
}
